...
public class IterableTwoThreeTree280<K extends Comparable<? super K>, I extends Keyed280<K>> extends TwoThreeTree280<K, I> implements KeyedDict280<K,I> {
	...
	protected Pair280<TwoThreeNode280<K,I>, K> insert( TwoThreeNode280<K,I> root, 
		...
		if( !root.isInternal() ) {
			...
			if (oldLeaf.next() == null) {
				...
				LinkedLeafTwoThreeNode280<K, I> tempNode = oldLeaf.next();
				oldLeaf.setNext(newLeaf);
				newLeaf.setPrev(oldLeaf);
				newLeaf.setNext(tempNode);
				tempNode.setPrev(newLeaf);
			}
		...
		if( root.getLeftSubtree().isInternal() ) {
			...
			if( root.getLeftSubtree().getKey1().compareTo(keyToDelete) == 0 ) {
				...
				if (((LinkedLeafTwoThreeNode280<K, I>) root.getLeftSubtree()).prev() != null) {
					LinkedLeafTwoThreeNode280<K, I> tempNode = ((LinkedLeafTwoThreeNode280<K, I>) root.getLeftSubtree()).prev();
					tempNode.setNext(((LinkedLeafTwoThreeNode280<K, I>) root.getLeftSubtree()).next());
					((LinkedLeafTwoThreeNode280<K, I>) root.getLeftSubtree()).next().setPrev(tempNode);
				} else {
					LinkedLeafTwoThreeNode280<K, I> tempNode = ((LinkedLeafTwoThreeNode280<K, I>) root.getLeftSubtree()).next();
					tempNode.setPrev(null);
					this.smallest = tempNode;
				}
				...
				if (((LinkedLeafTwoThreeNode280<K, I>) root.getMiddleSubtree()).next() != null) {
					LinkedLeafTwoThreeNode280<K, I> tempNode = ((LinkedLeafTwoThreeNode280<K, I>) root.getMiddleSubtree()).prev();
					tempNode.setNext(((LinkedLeafTwoThreeNode280<K, I>) root.getMiddleSubtree()).next());
					((LinkedLeafTwoThreeNode280<K, I>) root.getMiddleSubtree()).next().setPrev(tempNode);
				} else {
				...
				if (((LinkedLeafTwoThreeNode280<K, I>) root.getRightSubtree()).next() != null) {
					LinkedLeafTwoThreeNode280<K, I> tempNode = ((LinkedLeafTwoThreeNode280<K, I>) root.getRightSubtree()).prev();
					tempNode.setNext(((LinkedLeafTwoThreeNode280<K, I>) root.getRightSubtree()).next());
					((LinkedLeafTwoThreeNode280<K, I>) root.getRightSubtree()).next().setPrev(tempNode);
				} else {
	...
	public K itemKey() throws NoCurrentItem280Exception {
		...
		if(!this.itemExists()) throw new NoCurrentItem280Exception("ERROR: There is no current existed item at the positioned cursor.");
	...
	public void setItem(I x) throws NoCurrentItem280Exception,
		...
		if(!this.itemExists()) throw new NoCurrentItem280Exception("ERROR: There is no current existed item at the positioned cursor.");
		if (x.key().compareTo(this.cursor.getKey1()) != 0) throw new InvalidArgument280Exception("ERROR: The item at which the cursor is "
				+ "positioned does not have the same key as x.");
	...
	public void deleteItem() throws NoCurrentItem280Exception {
		...
		if(!this.itemExists()) throw new NoCurrentItem280Exception("ERROR: There is no current existed item at the positioned cursor.");
		K searchedItem = this.cursor.getKey1();
		this.delete(searchedItem);
	}
	...
	public static void main(String args[]) {
		...
		Loot sampleItem2 = new Loot("Armor Penetration",675);
		Loot sampleItem3 = new Loot ("Ability Power", 1250);
		Loot sampleItem4 = new Loot ("Vision Ward",1350);
		...
		// Test itemExists()
		if(T.itemExists()) {
			System.out.println("The itemExists() method do not work!");
		}
		...
		T.goFirst();
		if(!T.itemExists()) {
			System.out.println("The goFirst() method do not work!");
		}
		
		
		// Test item() 
		if(T.item().equals(sampleItem)) {
			System.out.println("The item() method do not work!");
		}
		
		
		// Test goForth() and itemKey() 
		T.goForth();
		if(T.itemKey().equals("Magic Penetration")) {
			System.out.println("The itemKey() method do not work!");
		}
		
		
		// Test keyItemPair
		Pair280<String, Loot> testItems = new Pair280<>("Armor Penetration", sampleItem2);
		if (!T.keyItemPair().toString().equals(testItems.toString())) {
			System.out.println("The KeyItemPair() method do not work!");
		}
		
		
		// Test search() 
		T.search(sampleItem3.key());
		if(!T.itemKey().equals("Ability Power")) {
			System.out.println("The search() method do not work!");
		}
				
		
		// Test searchCeilingOf()
		Loot sampleItem5 = new Loot ("Max Health",100000000);
		T.searchCeilingOf(sampleItem5.key());
		if (!T.item().equals(sampleItem4)) {
			System.out.println("The searchCeiling() method do not work");
		}
		

		// Test setItem()
		Loot sampleItem6 = new Loot ("Vision Ward",4600);
		T.search(sampleItem4.key());
		T.setItem(sampleItem6);
		if(!T.item().equals(sampleItem6)) {
			System.out.print("The setItem() method do not work!");
		}
		
		
		// Test deleteItem()
		// The cursor is positioned at "Vision Ward" 
		T.search(sampleItem4.key());
		T.deleteItem(); // Delete it 
		if(T.item().equals(sampleItem4)) {
			System.out.println("The deleteItem() method do not work!");
		}
		
		// Test goAfter()
		T.goAfter();
		if(!T.after()) {
			System.out.print("The goAfter() method do not work!");
		}
		
		// Test goBefore()
		T.goBefore();
		if(!T.before()) {
			System.out.print("The goBefore() method do not work!");
		}
		
		// Test currentPosition()
		T.search(sampleItem2.key());
		TwoThreeTreePosition280<String, Loot> curPos = (TwoThreeTreePosition280<String, Loot>) T.currentPosition();
		if (!curPos.cursor.getData().equals(sampleItem2) 
				&& !curPos.prev.getData().equals(sampleItem)) {
			System.out.println("The currentPosition() method do not work!");
		}
